...
def test_positive_p10_check(prepare_graph_positive: nx.Graph):
    """check is not None if the graph is valid"""
    assert ProductionP10(prepare_graph_positive).check is not None

#TODO write more tests

def test_negative_p10_check(prepare_graph_negative: nx.Graph):
    """check is None if the graph is not valid"""
    assert ProductionP10(prepare_graph_negative).check is None

def test_negative_p10_apply(prepare_graph_negative: nx.Graph):
    """apply creates new node in graph due to the graph is invalid"""
    ProductionP10(prepare_graph_negative).apply()
