...
def visualize_graph(graph: nx.Graph, title: str = 'Graph Visualization'):
    """Visualize the graph."""
    colors = ['lightblue' if data.get('label') == 'Q' else 'lightgreen' for node, data in graph.nodes(True)]
    labels = {}
    for node, data in graph.nodes(True):
        if data.get('label') == 'Q':
            labels[node] = f'R = {data.get('R')}'
        else:
            labels[node] = f'x = {data.get('x')}\ny = {data.get('y')}\nh = {data.get('h')}'
    edge_labels = {}
    ...
    for point, data in graph.nodes(True):
        if data.get('label') == 'Q':
            x = fmean([graph.nodes[neighbor].get('x') for neighbor in graph.neighbors(point)])
            y = fmean([graph.nodes[neighbor].get('y') for neighbor in graph.neighbors(point)])
            positions[point] = (x, y)
        else:
            positions[point] = (data.get('x'), data.get('y'))

    plt.figure(figsize=(20, 20))
    nx.draw(graph, pos=positions, with_labels=True, labels=labels, node_color=colors, node_size=3000)
...
def prepare_valid_test_graph() -> nx.Graph:
    ...
    G.add_edges_from([
        ...
        ('v:1.0:0.0', 'v:1.0:1.0', {'label': 'E', 'B': 0}),
